...
class MasterItemsController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($kode != '') $data_search = $data_search->where('kode', $kode);
        if ($nama != '') $data_search = $data_search->where('nama', 'LIKE', '%' . $nama . '%');
        if ($hargamin != '') $data_search = $data_search->where('harga_beli', '>=', $hargamin);
        if ($hargamax != '') $data_search = $data_search->where('harga_beli', '<=', $hargamax);
    ...
    public function formView($method, $id = 0)
    {
        ...
        $data['item'] = $item;
        $data['categories'] = \App\Models\MasterCategory::all();
        $data['method'] = $method;
